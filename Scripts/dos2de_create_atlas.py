...
<save>
    <header version="2" />
    <version major="3" minor="6" revision="4" build="1" />
...
for img in images:
	...
	u1 = numpy.clip(float(((icon_size * x) / atlas_size) + padding), 0, 1.0)
	v1 = numpy.clip(float(((icon_size * y) / atlas_size) + padding), 0, 1.0)
	u2 = numpy.clip(float(((icon_size * (x + 1)) / atlas_size) - padding), 0, 1.0)
	v2 = numpy.clip(float(((icon_size * (y + 1)) / atlas_size) - padding), 0, 1.0)
	icon = Icon(
		img.resolve(),
		x * icon_size,
		y * icon_size,
