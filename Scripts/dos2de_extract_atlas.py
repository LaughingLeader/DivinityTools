...
class Icon():
    def __init__(self, atlas, name, u1, v1, u2, v2):
        ...
        self.v2 = float(v2)
        self.rect = (
            numpy.floor(atlas.image_width * (self.u1)), 
            numpy.floor(atlas.image_height * (self.v1)), 
            numpy.ceil(atlas.image_width * (self.u2 - atlas.icon_offset)),
            numpy.ceil(atlas.image_height * (self.v2 - atlas.icon_offset))
        )
        if mapkey == "Skill_Water_FreezingTouch":
            print(u1,v1,u2,v2)
            print(self.rect)
    ...
    if os.path.exists(texture_path):
        ...
        for icon in atlas.icons:
            ...
            if Path(filename).exists() == True:
                filename = os.path.join(output_dir, icon.name + "(01).png")
